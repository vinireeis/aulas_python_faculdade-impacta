...
'''
    ...
    Vamos agora começar o tinder em si, permitindo a um(a) usuário(a)
    demonstrar interesse ("dar like")
    em outro usuário

    O plano é definir
    adiciona_interesse(id_interessado, id_alvo_de_interesse)

    Executar executar adiciona_interesse(9,3) significa que 9 está
    interessado(a) em 3

    a idéia é que temos o usuário de id 9 dizendo que tem interesse em
    conversar com o(a) usuário(a) de id 3.
...
'''
    ...
    Detalhes:
    * Remover um interesse pode "desligar" um match. A gostava de B, B gostava
    de A existia o match. B remove o interesse, nem A nem B tem match
    * Resetar ainda tem que funcionar
    * Buscar match de pessoa invalida ainda tem que dar raise na excessão
    NotFoundError
    '''
...
'''
    ...
    Se os dois perfis tem essas características (sexo e buscando) podemos
    verificar a compatibilidade

    Se você rodar adiciona_interesse(id1,id2) mas o buscando de 1 não for
    compativel com o sexo de 2 a sua função deve dar raise em uma excessão do
    tipo IncompatibleError
    '''
...
class TestStringMethods(unittest.TestCase):
    ...
    def test_p1_01_adiciona_pessoas(self):
        ...
        # crio fernando e roberto. Se houver algum Exception, abortará a
        # execução e você verá no teste
    ...
    def test_p1_04_pessoa_invalida(self):
        ...
        try:
            ...
            # e, portanto, a linha seguinte nao deve rodar. Se rodar, seu
            # código não deu o erro necessário, meu teste vai falar isso
            self.fail("uma Exception do tipo NotFoundError deveria ter ocorrido")
        except i.NotFoundError:
            # se a linha ZZZ deu o erro esperado, nao preciso fazer mais nada,
            # você passou no teste
            pass
        except:
            # a Excessão não foi a que eu queria
            self.fail('uma Exception do tipo NotFoundError deveria ter'
                      'ocorrido, mas ocorreu alguma outra')

    '''
    Esse teste verifica se a chamada adiciona_interesse(id_interessado,
    id_alvo_de_interesse)
    ...
    def test_p2_00_interesse_com_pessoas_validas(self):
        ...
        try:
            ...
            self.fail("adiciona interesse devia ter dado erro, mas nao deu."
                      " Pessoas ainda nao existem")
        except i.NotFoundError:
        ...
        i.adiciona_interesse(3, 9)
        i.adiciona_interesse(9, 3)
        # se desse pau nas funcoes acima, o teste te avisaria. Se nao avisou,
        # deu tudo certo
    ...
    def test_p4_01_match_incompativel(self):
        ...
        try:
            i.adiciona_interesse(3, 9)
            # se passar dessa linha sem dar erro, sua adiciona interesse nao
            # verificou a compatibilidade
